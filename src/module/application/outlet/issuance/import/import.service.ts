...
import {
    IssuanceMatrix,
    IssuanceOutletSummary,
    IssuancePreviewRow,
    ResponseIssuancePreviewDTO,
    ResponseIssuancePreviewDetailDTO,
} from "./import.schema.js";
import { IssuanceImportCacheService } from "./import.cache.js";
import { periodsFromRows, unpivotIssuance } from "./import.parser.js";

const LOCK_PREFIX = "issuance:import:lock:";
const LOCK_TTL_SECONDS = 120;
const MAX_LOGICAL_ROWS = 50_000;

type CachePayload = ResponseIssuancePreviewDetailDTO & { status: "preview" | "executing" };
...
export class IssuanceService {
    static async preview(matrix: IssuanceMatrix): Promise<ResponseIssuancePreviewDTO> {
        const rows = unpivotIssuance(matrix);
        ...
        const mergedRows = new Map<string, IssuancePreviewRow>();
        for (const row of rows) {
        ...
        await IssuanceImportCacheService.save(import_id, payload);
        return this.publicPreview(payload);
    }

    private static summarize(rows: IssuancePreviewRow[], outlets: Map<string, { id: number; code: string; name: string }>): IssuanceOutletSummary[] {
        const grouped = new Map<string, IssuancePreviewRow[]>();
    ...
    private static publicPreview(payload: CachePayload): ResponseIssuancePreviewDTO {
        const { rows: _rows, createdAt: _createdAt, status: _status, ...preview } = payload;
        return preview;
    }

    static async getPreview(import_id: string): Promise<ResponseIssuancePreviewDetailDTO> {
        const cache = await IssuanceImportCacheService.get<CachePayload>(import_id);
    ...
    static async execute(import_id: string): Promise<{ import_id: string; total: number; periods: CachePayload["periods"] }> {
        ...
        try {
            ...
            await IssuanceImportCacheService.save(import_id, { ...cache, status: "executing" });
            try {
                await this.replaceLedger(validRows, cache.periods, import_id);
                await IssuanceImportCacheService.remove(import_id);
                return { import_id, total: validRows.length, periods: cache.periods };
            } catch (error) {
                await IssuanceImportCacheService.save(import_id, cache);
                throw error;
            }
        } finally { await releaseLock(import_id); }
    }

    private static async replaceLedger(rows: IssuancePreviewRow[], periods: CachePayload["periods"], importBatchId: string): Promise<void> {
        await prisma.$transaction(async (tx) => {
            const periodWhere = periods.map((period) => ({ month: period.month, year: period.year }));
            await tx.outletIssuance.deleteMany({ where: { OR: periodWhere } });
            await tx.outletIssuance.createMany({ data: rows.map((row) => ({
            ...
            const aggregates = await tx.outletIssuance.groupBy({ by: ["outlet_id", "product_id", "month", "year"], where: { OR: periodWhere }, _sum: { quantity: true } });
