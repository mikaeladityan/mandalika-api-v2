...
import { IssuanceCell, IssuanceMatrix, RequestIssuanceExecuteDTO } from "./import.schema.js";
import { IssuanceService } from "./import.service.js";

const XLSX_MIME = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

async function parseXlsx(buffer: Buffer<ArrayBufferLike>): Promise<IssuanceMatrix> {
    const workbook = new ExcelJS.Workbook();
    // reason: ExcelJS's Node typings omit Buffer even though load() accepts it at runtime.
    await workbook.xlsx.load(buffer as unknown as ArrayBuffer);
    const sheet = workbook.worksheets[0];
    if (!sheet) throw new ApiError(400, "Sheet ISSUANCE tidak ditemukan");
    const matrix: IssuanceMatrix = [];
    sheet.eachRow((row, rowNumber) => {
        const values: IssuanceCell[] = [];
...
function parseCsv(buffer: Buffer): IssuanceMatrix {
    const raw = parse(buffer, { skip_empty_lines: false, relax_column_count: true, bom: true }) as string[][];
    return raw.map((row) => row.map((cell): IssuanceCell => cell === "" ? null : cell));
}
...
export class IssuanceController {
    static async preview(c: Context) {
        ...
        const matrix = mimetype === XLSX_MIME || filename.toLowerCase().endsWith(".xlsx")
            ...
            : parseCsv(buffer);
        return ApiResponse.sendSuccess(c, await IssuanceService.preview(matrix), 201);
    }

    static async getPreview(c: Context) {
        return ApiResponse.sendSuccess(c, await IssuanceService.getPreview(importIdFromContext(c)), 200);
    }

    static async execute(c: Context) {
        const body = c.get("body") as RequestIssuanceExecuteDTO;
        return ApiResponse.sendSuccess(c, await IssuanceService.execute(body.import_id), 200);
    }
}
