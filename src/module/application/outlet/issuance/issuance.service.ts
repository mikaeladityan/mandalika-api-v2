...
export class IssuanceService {
    ...
    static async grid(query: QueryIssuanceDTO) {
        ...
        const where = this.whereOf(query);
        const productIds = await prisma.outletIssuance.findMany({ where, distinct: ["product_id"], select: { product_id: true } });
        ...
        const [columnRecords, records] = await Promise.all([
            prisma.outletIssuance.findMany({ where, distinct: ["outlet_id", "date"], orderBy: [{ outlet: { code: "asc" } }, { date: "asc" }], select: { outlet_id: true, date: true, outlet: { select: { code: true, name: true } } } }),
            prisma.outletIssuance.findMany({ where: { ...where, product_id: { in: pageProductIds } }, orderBy: [{ outlet: { code: "asc" } }, { date: "asc" }, { product: { code: "asc" } }], include: ISSUANCE_INCLUDE }),
        ]);
