...
import { orderProductIdsByForecast } from "../../outlet/shared/forecast-product-order.js";
...
function sortDetails(rows: SafetyStockDetailRow[], query: QuerySafetyStockDTO, productRank: Map<number, number>) {
    ...
    rows.sort((left, right) => {
        ...
        const rank = (productRank.get(left.product_id) ?? Number.MAX_SAFE_INTEGER) - (productRank.get(right.product_id) ?? Number.MAX_SAFE_INTEGER);
        if (rank) return rank;
...
function sortSummary(rows: SafetyStockSummaryRow[], query: Query, productRank: Map<number, number>) {
    const key = query.sortBy ?? "product_code";
    const direction = query.order ?? "asc";
    rows.sort((left, right) => {
        const status = statusRank(left.product_status) - statusRank(right.product_status);
...
async function buildRows(query: QuerySafetyStockDTO) {
    ...
    const orderedProductIds = await orderProductIdsByForecast(productRows.map((product) => product.id));
    return { rows, productRank: new Map(orderedProductIds.map((id, index) => [id, index])), ...period };
}
...
export class SafetyStockService {
    ...
    static async summary(query: QuerySafetyStockSummaryDTO) {
        const { rows, productRank, period_start, period_end } = await buildRows({ ...query, outlet_id: undefined });
        ...
        sortSummary(data, query, productRank);
        const start = (query.page - 1) * query.take;
