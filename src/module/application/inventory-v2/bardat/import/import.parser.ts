...
const asText = (cell: BardatCell): string =>
...
function parseDate(cell: BardatCell): string | null {
...
function parseQuantity(cell: BardatCell): { value: number | null; blank: boolean } {
...
export function unpivotBardat(matrix: BardatMatrix): BardatPreviewRow[] {
    if (matrix.length < 3) throw new Error("Format BARDAT tidak memiliki baris header yang cukup");
    ...
    if (tokoRow < 0 || tanggalRow < 0 || tanggalRow <= tokoRow) {
        throw new ApiError(400, "Header BARDAT TOKO/TANGGAL tidak ditemukan atau urutannya salah");
    }
    ...
    if (productHeaderRow < 0 || productCodeCol < 0 || firstDateCol < 0) {
        throw new ApiError(400, "Kolom PRODUCT CODE atau tanggal BARDAT tidak ditemukan");
    }
...
export function periodsFromRows(rows: BardatPreviewRow[]): BardatPeriod[] {
    const values = new Map<string, BardatPeriod>();
