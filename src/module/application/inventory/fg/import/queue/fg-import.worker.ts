...
import type { FGImportJobData } from "./fg-import.queue.js";
...
export async function processFGImportJob(import_id: string, onProgress: (pct: number) => Promise<void>) {
    ...
    await redisClient.expire(ImportCacheService.key(CACHE_PREFIX, import_id), PROCESSING_TTL_SECONDS);
    ...
    const maps: MasterMaps = await prisma.$transaction(async (tx) => {
        const [typeIds, sizeIds] = await Promise.all([
            Promise.all(
...
export function createFGImportWorker() {
    const worker = new Worker<FGImportJobData>(
        FG_IMPORT_QUEUE_NAME,
        async (job) => {
            const { import_id } = job.data;
            return processFGImportJob(import_id, async (pct) => {
    ...
    worker.on("completed", (job) => {
        logger.info("FG import job completed", { jobId: job.id, result: job.returnvalue });
    });

    worker.on("failed", (job, err) => {
        logger.error("FG import job failed", {
            jobId: job?.id,
