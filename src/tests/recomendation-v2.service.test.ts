...
describe("RecomendationV2Service - Override Features", () => {
    ...
    it("keeps General recommendation zero when stock covers General needs", async () => {
        ...
        vi.mocked(prisma.$queryRaw)
            ...
            .mockResolvedValueOnce([{
                material_id: 7, barcode: "RM-7", material_name: "Shared RM",
                current_stock: 100, open_po: 0, safety_stock_x_resep: 0,
                recommendation_quantity: 0, total_forecast_horizon_dynamic: 50,
                forecast_needed: 50, sales_data: [], needs_data: [], po_data: [],
                work_order_data: { horizon: 1 },
            }])
            .mockResolvedValueOnce([{ count: 1 }]);
        const result = await RecomendationV2Service.list({
            type: "ffo", month: 9, year: 2026, page: 1, take: 10,
            sales_months: 3, forecast_months: 3, po_months: 3,
        });
        expect(result.data[0]).toMatchObject({
