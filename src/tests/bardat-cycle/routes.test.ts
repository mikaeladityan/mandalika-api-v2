...
describe("cycle API", () => {
    it.each([6, 7, 8, 9, 10])("projects April history into month %s through now plus one month", async month => {
        ...
        expect(mocks.receipts).toHaveBeenLastCalledWith(expect.objectContaining({ where: expect.objectContaining({ date: { gte: new Date("2026-02-01"), lt: new Date("2026-05-01") } }) }));
    });
    it("stops recommendations beyond now plus one month, while preserving overrides", async () => {
        ...
        expect(mocks.receipts).toHaveBeenCalledTimes(1);
    });
    ...
    it("recommends frequent history for an empty future month and prioritizes overrides", async () => {
        ...
        expect(mocks.receipts).toHaveBeenLastCalledWith(expect.objectContaining({ where: expect.objectContaining({ quantity: { gt: 0 }, date: { gte: new Date("2026-02-01"), lt: new Date("2026-05-01") } }) }));
    });
    ...
    it("keeps outlets visible before any shipping pattern is configured", async () => {
        ...
        expect((await res.json()).data).toEqual({ outlets: [{ id: 1, code: "A", name: "Toko A" }], rules: [], entries: [], latest_period: null, recommendation_until: "2026-10" });
