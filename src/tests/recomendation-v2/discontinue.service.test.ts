...
describe("Discontinue recipe anchor", () => {
    ...
    it("aggregates recipe-derived needs for shared RM before applying stock", async () => {
        db.discontinueNeedAnchor.findMany.mockResolvedValue([
            { product_id: 1, anchor_material_id: 10, quantity: decimal(10) },
            { product_id: 2, anchor_material_id: 20, quantity: decimal(40) },
        ]);
        const result = await DiscontinueService.purchases(9, 2026);
    ...
    it("removes the General addition when no anchors remain", async () => {
        ...
        expect(await DiscontinueService.purchases(10, 2026)).toEqual(new Map());
