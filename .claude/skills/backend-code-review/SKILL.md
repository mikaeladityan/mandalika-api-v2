...
description: Review backend code for quality, security, maintainability, and best practices based on established checklist rules. Use when the user requests a review, analysis, or improvement of backend files (e.g., `.ts`) under the `api/` directory. Do NOT use for frontend files (e.g., `.tsx`). Supports pending-change review, code snippets review, and file-focused review.
---
...
Use this skill whenever the user asks to **review, analyze, or improve** backend code (e.g., `.ts`) under the `api/` directory. Supports the following review modes:
...
- **File-focused review**: when the user points to specific files and asks for a review of those files (one file or a small, explicit set of files, e.g., `api/src/server.ts`).
...
- db schema design: if the review scope includes code/files under `api/prisma/`, follow [references/db-schema-rule.md](references/db-schema-rule.md) to perform the review
- architecture: if the review scope involves controller/service/schema/routes layering, dependency direction, or moving responsibilities across modules, follow [references/architecture-rule.md](references/architecture-rule.md) to perform the review
- service data access: if the review scope contains database operations (ORM or Raw SQL) and is not encapsulated within the appropriate service methods under `api/src/module/application/`, follow [references/repositories-rule.md](references/repositories-rule.md) to perform the review
- prisma query patterns: if the review scope involves Prisma Client usage, `$transaction` blocks, concurrency safeguards, or raw SQL usage, follow [references/sqlalchemy-rule.md](references/sqlalchemy-rule.md) to perform the review
