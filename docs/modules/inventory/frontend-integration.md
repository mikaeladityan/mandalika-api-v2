...
| :----------- | :--------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------- | :--------- | :----------- |
| `fg`         | `src/module/application/inventory/fg/fg.schema.ts` → `RequestFGSchema` / `ResponseFGSchema` / `QueryFGSchema` / `BulkStatusFGSchema` / `StatusParamFGSchema` | `app/src/app/(application)/inventory/fg/server/inventory.fg.schema.ts` → `RequestFGDTO` / `ResponseFGDTO` / `QueryFGDTO` / `BulkStatusFGDTO`         | `@/shared/types` (`STATUS`, `GENDER`)      | 🚧 TBD     | FE belum ada |
