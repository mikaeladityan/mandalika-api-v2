...
| Stock Movement / FG          | `/api/app/inventory/monitoring/stock-movement/fg`             | `src/module/application/inventory/monitoring/stock-movement/fg`                 | ✅ Ready | [stock-movement/README.md](./stock-movement/README.md)             |
| Stock Movement / RM          | `/api/app/inventory/monitoring/stock-movement/rm`             | `src/module/application/inventory/monitoring/stock-movement/rm`                 | ✅ Ready | [stock-movement/README.md](./stock-movement/README.md)             |
...
| Stock Location / FG          | `/api/app/inventory/monitoring/stock-location/fg`             | `src/module/application/inventory/monitoring/stock-location/fg`                 | ✅ Ready | [stock-location/README.md](./stock-location/README.md)             |
| Stock Location / RM          | `/api/app/inventory/monitoring/stock-location/rm`             | `src/module/application/inventory/monitoring/stock-location/rm`                 | ✅ Ready | [stock-location/README.md](./stock-location/README.md)             |
...
> Modul lama `inventory-v2/monitoring/stock-card` di-replace oleh `stock-movement` di path baru — schema/service/controller direlokasi + di-rename dengan SQL safety + index trigram `raw_materials.barcode` ditambahkan. Per 2026-05-20 di-split menjadi dua sub-scope simetris (`fg` + `rm`) mengikuti pola `stock-distribution`. FE legacy masih hidup di `inventory-v2/monitoring/stock-card`; migrasi FE menjadi dua route menyusul.
> Modul `stock-location` per 2026-05-20 di-restruktur dari FG-only menjadi dua sub-scope simetris (`fg` + `rm`) — folder `fg/` untuk warehouse FG/outlet, folder `rm/` untuk warehouse RM. Helper period dipindah ke `_shared/period.helpers.ts`. Test suite 33/33 hijau.
